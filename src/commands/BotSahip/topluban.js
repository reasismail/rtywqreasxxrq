...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    const adoniscik = await message.lineReply(
        `${members
            .map((member, idx) => `**${idx + 1}. ${member.toString()}**`)
            .join("\n")}\nBu üyeleri banlamak istiyor musun?`,
    );
    await adoniscik.react(green);
    const collector = adoniscik.createReactionCollector(
        (reaction, user) =>
    ...
    collector.on("collect", async () => {
        await adoniscik.edit(`${green} ${members.length} adet kullanıcı başarıyla yasaklandı.`);
    ...
    collector.on("end", (_, reason) => {
        ...
        if (reason === "time")
            adoniscik.edit("10 saniye geçtiği için işlem iptal edildi.");
    });
