...
let member = message.guild.member(message.mentions.members.first() || message.guild.members.cache.get(args[0]) || message.author);
    ...
    return }
       ...
       let ekipRolu = "996887864392552608"
...
.setThumbnail(message.author.displayAvatarURL({ dynamic: true, size: 2048 }))
  ...
  let msg = await message.channel.send({ buttons : [ecdagit,tagrol,kayıtsızdagit], embed: adonis})
    ...
    let adonis = message.guild.members.cache.filter(member => !member.roles.cache.has(ayar.etkinlik) || !member.roles.cache.has(ayar.cekilis))
    button.reply.send(`
