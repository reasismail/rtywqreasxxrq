...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    let etiketli;
    if (conf.discrim && conf.discrim.length > 0) etiketli = invMember ? message.guild.members.cache.filter((m) => invMember.some((x) => x.userID === m.user.id) && m.user.discriminator.includes(conf.discrim)).size : 0;
    else etiketli = 0;
...
${kirmiziok} Günlük: \`${daily}\`, Haftalık: \`${weekly}\`, Taglı: \`${tagged}\`, Etiketli: \`${etiketli}\`
      `);
