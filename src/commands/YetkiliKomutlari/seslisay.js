    ...
    if(!conf.teyitciRolleri.some(oku => message.member.roles.cache.has(oku)) && !conf.sahipRolu.some(oku => message.member.roles.cache.has(oku)) && !message.member.hasPermission('ADMINISTRATOR')) 
{
  ...
  let discrim = message.guild.members.cache.filter(x => { return x.user.discriminator.includes(conf.discrim) && x.voice.channel}).size
...
${kirmiziok} Ses kanallarında **${discrim}** discimli yetkili var !
