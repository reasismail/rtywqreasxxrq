...
module.exports = async (member) => {
  ...
  if (member.user.discriminator.includes(ayar.discrim)) {
    await member.roles.add(ayar.ekipRolu)
    await member.roles.add(ayar.unregRoles)
  client.channels.cache.get(ayar.ekipLogChannel).send(`<@${member.id}> adlı kişi sunucumuza etiket taglı şekilde katıldı, isminde ${ayar.etiket} sembolü bulunuyor.`)
  ...
  var üyesayısı = member.guild.members.cache.size.toString().replace(/ /g, "    ")
        ...
        if(üs) {
          üyesayısı = üyesayısı.replace(/([0-9])/g, d => {
            return {
              '0': `<a:SAYIEMOJ_0:1040897056748875806>`,
              '1': `<a:SAYIEMOJ_1:1040899346247463013>`,
              '2': `<a:SAYIEMOJ_2:1040900982214758440>`,
              '3': `<a:SAYIEMOJ_3:1040901080973848598>`,
              '4': `<a:SAYIEMOJ_4:1040901115740426280>`,
              '5': `<a:SAYIEMOJ_5:1040901160288137236>`,
              '6': `<a:SAYIEMOJ_6:1040901194903736401>`,
              '7': `<a:SAYIEMOJ_7:1040901225723482124>`,
              '8': `<a:SAYIEMOJ_8:1040901253800132638>`,
              '9': `<a:SAYIEMOJ_9:1040901272750006282>`}[d];
            })
          }     
...
await bannedTag.findOne({ guildID: settings.guildID }, async ( err, res) => {
  ...
  if(res.taglar.some(x => member.user.username.includes(x))) { 
    await member.setRoles(ayar.jailRole)
...
kayitchannel.wsend(`
:tada:  ${member}  \`${member.guild.name}\`'e Hoş Geldin :tada: 
Seninle beraber sunucumuz ${üyesayısı} üye sayısına ulaştı.

Hesabın __${memberGün} ${memberAylar} ${memberTarih}__ tarihinde \`` + moment(member.user.createdTimestamp).fromNow() + `\` oluşturulmuş ${guvenilirlik ? `Şüpheli! ${red} ` : `Güvenli! ${green}` }\nKayıt işleminden sonra <#982946720856018954> kanalına göz atmayı unutmayın.

Tagımıza ulaşmak için herhangi bir kanala \`.tag\` yazabilirsiniz. \`\`\`Kayıt olduktan sonra kuralları okuduğunuzu kabul edeceğiz ve içeride yapılacak cezalandırma işlemlerini bunu göz önünde bulundurarak yapacağız. ${member.guild.name} \`\`\` `);
channel.wsend(`${member}, sunucuya katıldı! Davet Eden: **Özel URL** <:giris:1041764096967720960> `)
return }
...
member.setRoles(ayar.fakeAccRole)
} else {
...
Tagımıza ulaşmak için herhangi bir kanala \`.tag\` yazabilirsiniz. \`\`\`Kayıt olduktan sonra kuralları okuduğunuzu kabul edeceğiz ve içeride yapılacak cezalandırma işlemlerini bunu göz önünde bulundurarak yapacağız. ${member.guild.name} \`\`\` `);
channel.wsend(`${member}, ${invite.inviter.tag} davetiyle katıldı! (**${total}**) <:giris:1041764096967720960> `)
